...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CherbstersAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/herbsters/herbsters/pull/10340, but that change has been
    // postponed until after https://github.com/herbsters/herbsters/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-herbsters-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CherbstersAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CherbstersAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CherbstersAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CherbstersAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CherbstersAddress(address).ToString());
}
 ...
 * https://herbsterstalk.org/?topic=54527, or
 * https://github.com/herbsters/herbsters/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CherbstersAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CherbstersAddress(dest).ToString(), key);
}
