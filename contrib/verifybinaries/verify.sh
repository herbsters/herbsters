...
###   herbsterscore.org and herbsters.org and compares them.
...
WORKINGDIR="/tmp/herbsters_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://herbsterscore.org"
HOST2="https://herbsters.org"
BASEDIR="/bin/"
VERSIONPREFIX="herbsters-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'herbsters-',
...
if [ $? -ne 0 ]; then
   echo "herbsters.org failed to provide signature file, but herbsterscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "herbsters.org and herbsterscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the herbsters Core binary release signing key installed?"
   fi
