...
The RPM spec file provided here is for herbsters-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://herbsters.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[herbsters git repository](https://github.com/herbsters/herbsters) but are not part of
...
Currently only `Source10` is used, for the example `herbsters.conf` file.
...
in the herbsters git repository. At present only an SVG version of the herbsters
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the herbsters
...
needed when 0.12.1 is released, a proper fix is already in the herbsters git
master branch.
...
idea to build herbsters Core against a static version of that library compiled
...
The `herbsters.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `herbsters-server` package. At this point, most RPM based
...
The `herbsters-server` package should have SELinux support. How to properly do
...
herbsters-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/herbsters/). He has been
packaging herbsters for Fedora at least since 2012.
...
4. I renamed the `herbsters` package that contains the Qt GUI to `herbsters-core` as
that appears to be how the general population refers to it, in contrast to
`herbsters-xt` or `herbsters-classic`. I wanted to make sure the general population
